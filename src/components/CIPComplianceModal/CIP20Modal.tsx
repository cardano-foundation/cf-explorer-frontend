...
const CIP20Modal: React.FC<TCIP20ComplianceModalProps> = (props) => {
  ...
  const tokenMaps = useMemo(() => {
    if (isEmpty(data)) {
      return [{ requireProperties: DEFAULT_CIP20_REQUIRE, tokenName: null, optionalProperties: [] }];
    } else {
      return Object.keys(data).map((key) => {
        ...
        if (data[key].requireProperties?.[0]["property"] === "music_metadata_version" && inValidVersion) {
          return {
            requireProperties: [...DEFAULT_CIP20_REQUIRE, ...data[key].requireProperties],
