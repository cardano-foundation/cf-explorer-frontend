...
export default class EpochPanel extends WebApi {
  ...
  async verifySortField(field: string, sortType: string) {
    const blockList = util.format(itemLists2, EpochConstants.COLUMN_NAME[3], EpochConstants.COLUMN_NAME[3]);
    const totalOutputList = util.format(itemLists2, EpochConstants.COLUMN_NAME[6], EpochConstants.COLUMN_NAME[6]);
    switch (field) {
      case "Blocks":
