describe("User check Bookmark - Pool in My Profile", () => {
    it("User go to Dashboard and sign in, then verify the Bookmark - Pool in My Profile Screen", () => {
      ...
      cy.get('button').contains('Pool').click();
      cy.get('[data-testid="table-common"]').should('be.visible');
      cy.get('th').contains('Pool ID').should('be.visible');
