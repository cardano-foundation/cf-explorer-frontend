describe("User check Bookmark - Epoch in My Profile", () => {
  it("fake test", () => {
    cy.visit("/");
    cy.get('[data-testid="menu-button-dashboard"]').contains("Dashboard");
  });
  /*
    it("User go to Dashboard and sign in, then verify the Bookmark - Epoch in My Profile Screen", () => {
      ...
      cy.get('button').contains('Epoch').click();
      cy.get('[data-testid="table-common"]').should('be.visible');
      cy.get('th').contains('EPOCH #').should('be.visible');
