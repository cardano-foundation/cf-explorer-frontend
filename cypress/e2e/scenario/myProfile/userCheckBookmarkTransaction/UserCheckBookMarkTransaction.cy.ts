describe("User check Bookmark - Transaction in My Profile", () => {
  it("fake test", () => {
    cy.visit("/");
    cy.get('[data-testid="menu-button-dashboard"]').contains("Dashboard");
  });
  /*
    it("User go to Dashboard and sign in, then verify the Bookmark - Transaction in My Profile Screen", () => {
      ...
      cy.get('th').contains('Tnx Hash').should('be.visible');
