describe("User check information of stake address deregistration", () => {
  it("User go to Stake Addess Deregistration and click to first record, then verify the Stake Addess Detail screen", () => {
    ...
    cy.get(':nth-child(3) > .css-13ne0mf > .css-70qvj9 > .MuiBox-root').contains('Rewards available').should('be.visible'); 
    cy.get(':nth-child(4) > .css-13ne0mf > .css-70qvj9 > .MuiBox-root').contains(' Rewards withdrawn').should('be.visible'); 
    ...
    cy.get('[data-testid = "table-common"] th:nth-of-type(2)').should('be.visible').contains('Tx Hash');
    cy.get('[data-testid = "table-common"] th:nth-of-type(3)').should('be.visible').contains('Created At');
    cy.get('[data-testid = "table-common"] th:nth-of-type(4)').should('be.visible').contains('Block');
    cy.get('[data-testid = "table-common"] th:nth-of-type(5)').should('be.visible').contains('Fees');
    cy.get('[data-testid = "table-common"] th:nth-of-type(6)').should('be.visible').contains('ADA amount');
    cy.get('[data-testid = "table-common"] th:nth-of-type(7)').should('be.visible').contains('Token');
  });
});